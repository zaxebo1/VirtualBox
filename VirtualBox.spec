...
Source21:   VirtualBox-kmod.spec.tmpl
Source22:   VirtualBox-OSE-kmod-1.6.4-kernel-variants.txt
...
echo "and publisher is:XXX%{publisher}XXX vendor is:XXX%{vendor}XXX"
...
# Warning hacking akmods in a new way EXPERIMENTAL
#the install -n akmod-VirtualBox
# replace infile vboxversion and vboxrelease before use rpmbuild
%global vrelease %(echo %{release} | sed -e 's/%{?dist}//')
sed -i 's/vboxversion/%{version}/g; s/vboxrelease/%{vrelease}/' %{_sourcedir}/VirtualBox-kmod.spec.tmpl
mkdir -p %{buildroot}/%{_usrsrc}/akmods/; \
rpmbuild --define "_sourcedir %{_sourcedir}" \
--define "_srcrpmdir %{buildroot}/%{_usrsrc}/akmods/" \
-bs --nodeps %{_sourcedir}/VirtualBox-kmod.spec.tmpl ; \
ln -s $(ls %{buildroot}/%{_usrsrc}/akmods/) %{buildroot}/%{_usrsrc}/akmods/VirtualBox-kmod.latest
...
%package -n akmod-VirtualBox
Summary:        Akmod package for VirtualBox kernel module(s)
Group:          System Environment/Kernel
Requires:       kmodtool
Requires:       akmods
Requires:       VirtualBox-kmodsrc = %{version}-%{release} xz time
# same requires and provides as a kmods package would have
Requires:       VirtualBox-kmod-common = %{?epoch:%{epoch}:}%{version}-%{release}
Provides:       VirtualBox-kmod = %{?epoch:%{epoch}:}%{version}-%{release}

%description -n akmod-VirtualBox
This package provides the akmod package for the VirtualBox kernel modules.

%posttrans -n akmod-VirtualBox
nohup /usr/sbin/akmods --from-akmod-posttrans --akmod VirtualBox &> /dev/null &

%files -n akmod-VirtualBox
%{_usrsrc}/akmods/*
...
- Add akmod-VirtualBox to this package. 
